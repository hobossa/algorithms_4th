...
/**
 *  This code is from edu.princeton.cs.algs4.TrieSET.
 *  Modified it to minimize memory usage. Add a method to check if a string is a prefix or a key,
 **/
 ...
 *  @author Kevin Wayne
 */
public class TrieSETEx implements Iterable<String> {
    ...
    public int getKeyInfo(String key) {
        if (key == null) throw new IllegalArgumentException("argument to contains() is null");
        Node x = get(root, key, 0);
        if (x == null) return 0;    // does not contain the key
        if (x.isString) return 1;   // It is a key
        return 2;                   // It isa prefix
    }
